...
def bc_type_to_json(boundary_conditions):
...
def twobox(input_file, suffix):
    ...
    prob_params = bc_type_to_json([
      ('d', dict(id=1, value=[0, 0])),
      ('n', dict(id=2, value=[0, -0.2]))
      ])
...
def twobox(input_file, suffix):
    multimesh = [
      dict(mesh = input_file, position = [0,0.00], scale = 1, body_id = 1), 
      dict(mesh = input_file, position = [0,-2-1e-3], scale = 2, body_id = 2)
    ]
  
    bnd_side = [
          dict(id=1, axis=-2, position=0.00-1e-4),
          dict(id=2, axis=2, position=0.995),
          # dict(id=3, center=[0,0.75], radius = 0.1),
    ]
...
def tableslide(top_file, bottom_file, suffix=''):
    multimesh = [
      dict(mesh = top_file, position = [0,0.00], scale = 1, body_id = 1), 
      dict(mesh = bottom_file, position = [0,0.00-2e-3], scale = 1, body_id = 2)
    ]
  
    bnd_side = [
          dict(id=1, axis=-2, position=0.00-1e-3),
          dict(id=2, axis=2, position=0.995),
          dict(id=3, box=[[0.5-1e-3, -5], [0.5+1e-3,+5]]),
    ]

    prob_params = bc_type_to_json([
      ('d', dict(id=1, value=[0, 0])),
      ('n', dict(id=2, value=[0, -0.1])),
      ('d', dict(id=3, value=[0, 0], dimension=[True,False])),
      ])

    export_params = dict(vis_mesh=os.path.basename(top_file) + suffix + '.res.vtu')
